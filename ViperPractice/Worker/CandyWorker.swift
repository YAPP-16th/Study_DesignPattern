...
class CandyAPIWorker : APIWorkerProtocol {
    func fetchCandy(callBack: (CandyEntity) -> Void) {
    ...
    func fetchChoco(callBack: (CandyEntity) -> Void) {
        let chocoEntity = CandyEntity(title: "지옥 초콜릿", description: "많은 사람들의 다이어트를 방해한 지옥에서 올라온 초콜릿입니다. 사랑스러운 단맛과 깔끔함의 조합, 이런저런 일로 수고한 나에게 달콤함을 주는건 어떨까요? 어서 주문하세요!", price: 150, imageName: "magic_chocolate")
